...
The maximum number of steps as an average of the steps in the five-minute intervals of all days is `r round(ymax, 0)` and is obtained in the interval `r xmax`.
...
nrow(act_withNA)
```
The original data set has `r nrow(act_withNA)` rows in which one of the variables has a missing value.
